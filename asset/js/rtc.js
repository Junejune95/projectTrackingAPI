...
window.addEventListener('load', () => {
  ...
  var tokens = window.location.search.split('?')[1].split('&');
  var result = {};
  ...
  if (!room) {
    ...
    // if (result.userType && result.userType == 'Teacher') {
    //   document.getElementById('room-comm').style.display = 'block';
    // }
    for (let i = 0; i < commElem.length; i++) {
      if (result.userType && result.userType != 'Teacher') {
        document.getElementById('pull-right').style.display = 'none';
      }
